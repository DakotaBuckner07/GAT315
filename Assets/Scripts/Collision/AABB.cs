...
public struct AABB
{
	...
	public void Draw(Color c, float width = 0.05f)
    {
		Lines.Instance.AddLine(new Vector2(min.x, min.y), new Vector2(max.x, min.y), c, width);
		Lines.Instance.AddLine(new Vector2(min.x, max.y), new Vector2(max.x, max.y), c, width);
		Lines.Instance.AddLine(new Vector2(min.x, min.y), new Vector2(min.x, max.y), c, width);
		Lines.Instance.AddLine(new Vector2(max.x, min.y), new Vector2(max.x, max.y), c, width);
    }
